...
async def get_notifications(user_id: int, db: Session = Depends(db_dependency), current_user: User = Depends(get_current_user)):
...
async def mark_notification_read(notif_id: int, db: Session = Depends(db_dependency), current_user: User = Depends(get_current_user)):
